...
| circleSteps     | 生成圆边框的步数，步数越多越近似圆，反之可以通过该参数绘制正多边形                                                         | `number`                                                                       | `64`        | [绘制正多边形](/example/circle/edges)                     |
...
| multiple        | 单次 enable 时，Draw 是否支持绘制多个绘制物                                                                                | `boolean`                                                                      | `true`      | [禁止绘制多个](/example/circle/multiple#始终最多绘制一个) |
