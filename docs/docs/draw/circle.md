...
| addMultiple     | 关闭后，Draw 中一次 enable，最多只能绘制一个 Polygon 绘制物               | boolean                                               | true   | [关闭绘制多个 1](/example/circle/multiple#单次-enable-最多绘制一个) |
| multiple        | 关闭后，Draw 中始终最多只能绘制一个 Polygon 绘制物                        | boolean                                               | true   | [关闭绘制多个 2](/example/circle/multiple#始终最多绘制一个)         |
...
| DrawEvent.clear     | 清空数据的回调       | (drawer: DrawCircle) => void                                                                                       |
