...
export interface ICircleDrawerOptions extends IBasePolygonDrawerOptions {
  createByDrag: boolean;
  createByClick: boolean;
  circleStep: 90;
  distanceText: IDistanceOptions;
}
...
export class CircleDrawer extends BasePolygonDrawer<ICircleDrawerOptions> {
  ...
  initData(data: IDrawerOptionsData): Partial<ISourceData> | undefined {
    ...
    if (polygon?.length) {
      const result = polygon.map((feature) => {
        ...
        // const radius =
        return this.initCirclePolygon([lng1, lat1], [lng2, lat2], {
  ...
  syncPolygonNodes(feature: IPolygonFeature) {
    ...
    const radius = distance(nodes[0], nodes[1], {
      units: 'meters',
    });
    const positions = coordAll(
      circle(nodes[0], radius, {
        units: 'meters',
        steps: this.options.circleStep,
      }),
    );
  ...
  initCirclePolygon(
    ...
    const radius = distance(node1, node2, {
      units: 'meters',
    });
    const positions = coordAll(
      circle(node1, radius, {
  ...
  createDrawRect(e: ISceneMouseEvent | ILayerMouseEvent) {
    ...
    const newFeature = this.initCirclePolygon([lng, lat], [lng, lat], {
      isActive: true,
      isDraw: true,
    });
